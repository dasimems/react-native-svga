...
internal final class ProtoReader {
    ...
    var hasMore: Bool { pos < bytes.count }
    ...
    func readBytes() throws -> Data {
        ...
        if len < 0 || pos + len > bytes.count { throw SvgaError("length-delimited overflow") }
    ...
    func readFloat() throws -> Float {
        if pos + 4 > bytes.count { throw SvgaError("truncated float") }
    ...
    func skip(wire: Int) throws {
        ...
        case Wire.lengthDelimited.rawValue: _ = try readBytes()
    ...
    private func advance(_ n: Int) throws {
        if pos + n > bytes.count { throw SvgaError("truncated fixed field") }
        pos += n
    }
}
